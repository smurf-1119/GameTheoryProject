...
class PPO:
    ...
    def __init__(self, run_dir=None):
        ...
        if self.use_cnn:
            ...
            self.actor_net = Actor(self.state_space,self.action_space,self.hidden_size,self.cnn).to(device)
            self.critic_net = Critic(self.state_space,self.hidden_size,self.cnn).to(device)
        self.buffer = []
        self.counter = 0
    ...
    def select_action(self, state, train=True):
        ...
        if train:
            action = c.sample() #sample 取出action
        else:
    ...
    def update(self, i_ep):
        state_array = np.array([t.state for t in self.buffer],dtype=np.float64)
        action_array = np.array([t.action for t in self.buffer],dtype=np.float64)
        state_temp = torch.from_numpy(state_array).to(device)
        action_temp = torch.from_numpy(action_array).view(-1,1).to(device)
        state = state_temp.float()
        action = action_temp.long()
        ...
        Gt = []
        for r in reward[::-1]:
            R = r + self.gamma * R #(离他越远影响越小)
            Gt.insert(0, R) #从t开始到游戏结束的r的reward
        ...
        for i in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))), self.batch_size, False):
                ...
                surr2 = torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * advantage #PPO2 algorithem
